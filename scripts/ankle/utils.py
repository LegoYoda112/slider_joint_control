...
def calcGammaYukiTan(alpha, beta, foot):
    ...
    Ax = Lx * cos(beta);
    Ay = Ly * cos(alpha) + Lx * sin(alpha) * sin(beta);
    Az = Ly * sin(alpha) - Lx * cos(alpha) * sin(beta);

    h1 = L * L - (Mx - Ax) * (Mx - Ax);
    i1 = R * R;
    j1 = (Az - Mz) / (My - Ay);
    l1 = (-Ay * Ay + My * My + h1 - i1 + Mz * Mz - Az * Az) / (2 * My - 2 * Ay);

    A1 = j1 * j1 + 1;
    B1 = 2 * (l1 - Ay) * j1 - 2 * Az;
    C1 = (l1 - Ay) * (l1 - Ay) + Az * Az - h1;

    z_L = (-B1 + sqrt(B1 * B1 - 4 * A1 * C1)) / (2 * A1);
    y_L = z_L * j1 + l1;

    gamma = -arctan2((z_L - Mz), (y_L - My));

    return gamma;

def calcGammaYuki(alpha, beta, foot):
    Lx = 65.0
    Ly = 103.0
    ...
    gamma = arcsin((z_L - Mz)/R);

    return gamma;

def calcGammaYuki_(alpha, beta, foot):
    Lx = 65.0
    Ly = 103.0
...
def FK(angles, gammaL, gammaR):
    alpha = angles[0]
    beta = angles[1]

    errorL =  calcGammaPaper(alpha, beta, 'left') - gammaL
    errorR =  calcGammaPaper(alpha, beta, 'right') - gammaR
